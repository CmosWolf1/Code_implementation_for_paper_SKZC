...
# from .generateIoU import find_connected_components_in_heatmap, generateHeatmap, generateROIAnchors, drawAnchors
...
#
#         outputs_class, outputs_coord = self.head(backbone_feats, x, t, None)
#
#         x_start = outputs_coord[-1]  # (batch, retnum_proposals, 4) predict boxes: absolute coordinates (x1, y1, x2, y2)
...
#
#         # IM = images.tensor
#         # size = [images.image_sizes[0][0], images.image_sizes[0][1]]
#         # IM = np.array(batched_inputs[0]['image'].permute(1, 2, 0).to(dtype=int).numpy(), dtype=np.uint8)
#         # IM = Image.fromarray(IM, 'RGB')
#         # attention = generateHeatmap(IM, size)
#         # plt.imsave(fname='image.jpg', arr=attention, format='jpg')
#         # IM = cv2.imread("image.jpg")
#         # # maxVal = max(max(row) for row in attention)
#         # # IM = cv2.applyColorMap((attention / maxVal * 255).astype(np.uint8), cv2.COLORMAP_WINTER)
#         # # box = [[0, 0, IM.shape[1], IM.shape[0]]]
#         # box = find_connected_components_in_heatmap(IM)
#         # k=images_whwh[:, None, :]
#         # img, self.num_proposals = generateROIAnchors(box, [IM.shape[0], IM.shape[1]], k)
#
...
#         # img = torch.cat((img, im), 1)
#         # img = img[0].to(dtype=int).tolist()
#         # drawAnchors(IM, img, (0, 255, 0))
#         # cv2.imshow("boxes", IM)
#         # cv2.waitKey(0)
...
class DiffusionDet(nn.Module):
    ...
    def preprocess_image(self, batched_inputs):
        ...
        Normalize, pad and batch the input images.
        """
        # plt.imshow(batched_inputs[0]['image'].to(torch.uint8).numpy().transpose(1, 2, 0))
        # plt.show()
        ...
        # bI = batched_inputs[0]['image'].to(torch.uint8).numpy().transpose(1, 2, 0)
        # imG = cv2.cvtColor(bI, cv2.COLOR_RGB2BGR)
        # cv2.imshow('try', imG)
        # cv2.waitKey(0)
